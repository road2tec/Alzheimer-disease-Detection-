...
const Results = ({ result }) => {
    ...
    const getTheme = () => {
        if (prediction === 'AD') return { bg: 'bg-red-50', text: 'text-red-700', border: 'border-red-100', icon: AlertTriangle, label: 'High Risk' };
        if (prediction === 'MCI') return { bg: 'bg-amber-50', text: 'text-amber-700', border: 'border-amber-100', icon: TrendingUp, label: 'Early Signs' };
        return { bg: 'bg-emerald-50', text: 'text-emerald-700', border: 'border-emerald-100', icon: CheckCircle, label: 'Healthy' };
    };
        ...
        >
            ...
            <div className={`p-6 rounded-3xl border ${theme.bg} ${theme.border} lg:col-span-1 shadow-sm flex flex-col justify-between`}>
                <div>
                    ...
                    <div className={`text-3xl font-black ${theme.text} mb-4 uppercase`}>{theme.label}</div>
                    <p className="text-slate-600 text-sm leading-relaxed">
                        {prediction === 'AD' ? 'Signs of high risk for Alzheimer\'s health found.' :
                            prediction === 'MCI' ? 'Early signs of memory issues found. We suggest talking to a doctor.' :
                                'Your results look healthy. No issues found.'}
                    </p>
                </div>

                <div className="mt-8">
                    <div className="flex justify-between items-end mb-2">
                        <span className="text-xs font-bold text-slate-500 uppercase">AI Score</span>
                ...
                <div className="flex flex-col md:flex-row gap-8 items-center">
                    ...
                    <div className="flex-1 space-y-5">
                        <div className="bg-slate-50 p-6 rounded-2xl border border-slate-100">
                            <h5 className="font-bold text-slate-800 mb-2 text-sm uppercase tracking-wide">AI Map Analysis</h5>
                            <p className="text-xs text-slate-600 leading-relaxed">
                                "The colored map shows where the AI looked most. The warmer colors (red/orange) show the parts of the brain that the AI found most important for your result."
                            </p>
                        </div>
